...
test.describe("Bandcamp Discovery Player - extended E2E", () => {
      ...
      if (!(HTMLMediaElement.prototype as any)._hasMockedPlayback) {
        ...
        Object.defineProperty(HTMLMediaElement.prototype, "paused", {
          get() {
            ...
            return (this as any)._mockPaused === undefined
              ? true
              : (this as any)._mockPaused;
          },
          configurable: true,
        });
  ...
  test("full page load → albums displayed → click play → audio starts", async ({
    page,
  }) => {
    ...
    // Ensure audio element exists in #playe. Native controls are off, so the element is present but not rendered
    // the transport UI is what's visible
    await expect(page.locator("#player audio")).toBeAttached();
    await expect(page.locator("#player .track-scrubber")).toBeVisible();

    // Trigger play via page.evaluate to call the overridden play()
    await page
      .locator("#player audio")
      .evaluate((el: HTMLMediaElement) => el.play());

    // Verify audio is no longer paused
    const paused = await page
      .locator("#player audio")
      .evaluate((el: HTMLMediaElement) => el.paused);
  ...
  test("keyboard navigation (Q/E) cycles through albums", async ({ page }) => {
    await page.route("**/api/albums**", (route) => {
      route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(page1.concat(page2)),
      });
    });
  ...
  test("volume controls (ArrowUp/ArrowDown) change volume", async ({
    page,
  }) => {
    await page.route("**/api/albums**", (route) =>
      route.fulfill({
    ...
    await page.goto("/");
    await page.waitForSelector("#player audio", { state: "attached" });
    ...
    const afterUp = await page
      .locator("#player audio")
      .evaluate((a: HTMLAudioElement) => +a.volume.toFixed(2));
  ...
  test("error state display when API fails (500)", async ({ page }) => {
    ...
    const errText = await page
      .locator("#error-overlay .error-message")
      .textContent();
