...
import { findMatchingGenres } from "../../public/js/ui/genre-dropdown";
...
describe("findMatchingGenres", () => {
  it("matches partial genre names", () => {
    expect(findMatchingGenres("ambient")).toContain("dark-ambient");
  });

  it("matches fuzzy subsequences and ranks close matches first", () => {
    const matches = findMatchingGenres("dmbs");
    expect(matches[0]).toBe("drum-bass");
  });

  it("returns no matches for unrelated input", () => {
    expect(findMatchingGenres("zzzzzz")).toEqual([]);
  });
});
