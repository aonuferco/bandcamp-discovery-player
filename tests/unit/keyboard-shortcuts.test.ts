...
`;
  ...
  const mockAlbum = {
    ...
    artist: "Old",
    img: "",
    link: "https://test.bandcamp.com",
    stream_url: "https://cdn.bandcamp.com/stream.mp3",
    featured_track: null,
    label_name: null,
    track_count: 0,
    band_name: "",
  ...
  it('navigates to next album on "e"', async () => {
    ...
    const albums = Array.from({ length: 5 }, (_, i) => ({
      ...mockAlbum,
      link: `https://test${i + 1}.bandcamp.com`,
    }));
  ...
  it('navigates to previous album on "q"', async () => {
    ...
    const showAlbumSpy = vi.spyOn(controller.ui, "showAlbum");
    fireKey("q");
    // prevAlbum runs through the navigation queue — wait a tick for it to resolve
    await new Promise((r) => setTimeout(r, 10));
    expect(showAlbumSpy).toHaveBeenCalled();
  });
