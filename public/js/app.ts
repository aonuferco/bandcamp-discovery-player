...
export const createAppController = (): AppController => {
  ...
  let autoSkipTimeout: number | undefined;
  ...
  if (ui.elements.player) {
    ...
    audioController.onError(() => {
      ...
      ui.showToast("Track unavailable — skipping to the next album", "error");
      if (state.getAlbums().length === 0) return;
      if (autoSkipTimeout !== undefined) {
        window.clearTimeout(autoSkipTimeout);
      }
      autoSkipTimeout = window.setTimeout(() => {
        autoSkipTimeout = undefined;
        void nextAlbum();
      }, 2000);
    });
  }
  ...
  const fetchAlbums = async (page = 1) => {
    ...
    try {
      ...
      if (state.getAlbums().length === 0) {
        ui.showError(
          `Failed to load albums: ${errMsg}. Select Retry to try again.`,
        );
      } else {
  ...
  const retryFetch = async () => {
    if (ui.elements.retryBtn) {
      ui.elements.retryBtn.disabled = true;
      ui.elements.retryBtn.textContent = "Retrying…";
    }
    if (ui.elements.loadingSpinner) {
      ui.elements.loadingSpinner.classList.remove("hidden");
    }
    ...
    try {
      ...
      if (ui.elements.retryBtn) {
        ui.elements.retryBtn.disabled = false;
        ui.elements.retryBtn.textContent = "Retry";
      }
    }
  };
  ...
  const setupEventListeners = () => {
    ...
    document
      .querySelectorAll<HTMLButtonElement>(".genre-chip")
      .forEach((chip) => {
        chip.addEventListener("click", () => {
          const genre = chip.dataset["genre"];
          if (genre) void selectGenre(genre);
        });
      });
